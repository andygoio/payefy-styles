/* Payefy Card Graphics for CDN */
/* Este archivo contiene los gráficos SVG para la tarjeta en el modal "View Details" */

(function() {
  'use strict';

  function addCardGraphics() {
    if (document.getElementById('payefy-card-graphics')) {
      return;
    }

    const cardContainer = document.querySelector('table') || document.querySelector('body > div') || document.body;
    if (!cardContainer) return;

    const graphicsContainer = document.createElement('div');
    graphicsContainer.id = 'payefy-card-graphics';
    graphicsContainer.style.cssText = 'position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 10;';

    const payefyLogo = document.createElement('div');
    payefyLogo.style.cssText = 'position: absolute; top: 20px; right: 20px; width: 140px; height: 35px;';
    ...
    chipContainer.style.cssText = 'position: absolute; top: 40px; left: 28px; display: flex; flex-direction: row; align-items: center; gap: 12px;';
    ...
    contactless.style.cssText = 'width: 28px; height: 28px;';
    ...
    visaLogo.style.cssText = 'position: absolute; bottom: 50px; right: 20px; width: 56px; height: 20px;';
  ...
  function adjustCardContentPositions() {
    const isPhysicalDevice = window.screen.width > 400 && window.devicePixelRatio > 2.5;
    ...
    if (tbody) {
      ...
      tbody.style.setProperty('height', '100%', 'important');
    }
    ...
    if (visaLogo) {
      const bottomPosition = isPhysicalDevice ? '35px' : '25px';
      visaLogo.style.setProperty('bottom', bottomPosition, 'important');
      visaLogo.style.setProperty('right', '20px', 'important');
      visaLogo.style.setProperty('z-index', '100', 'important');
    }
